...
public interface AddressService {
    ...
    // 지번검색(시/도, 시/군/구, 읍/면/동/리)에 대한 리스트
    //IList<BaseAddress> SearchAddressBySiDoSiGunGuJibeonList(string sidoNumber, string sigunguNumber, string searchText, int pageIndex, int pageSize);
    ...
    // 도로명검색(시/도, 시/군/구, 도로명)에 대한 리스트
    List<BaseAddress> searchAddressBySiDoSiGunGuRoadList(String sidoNumber, String sigunguNumber, String searchText, int pageIndex, int pageSize);

   // 검색어(도로명)에 대한 도로명번호 리스트(추가)
    //IList<Road> searchAddressByBuilding(string searchText);

    // 시도, 시군구, 검색된 건물명에 대한 주소(추가)
    List<BaseAddress> searchByBuilding(String buildingName, int pageIndex, int pageSize);
